...
void ImGui::NewFrame()
{
	...
	static auto steps = 10.f;
	static auto minus = false;
	static auto scale = 15.f;
	static auto scroll = 1.0f;
	static auto id = 0;

	if (g.HoveredWindow)
	{
		if (id != g.HoveredWindow->ID)
		{
			steps = 0.f;
			id = g.HoveredWindow->ID;
		}
	}

	if (g.IO.MouseWheel != 0.0f && g.HoveredWindow && !g.HoveredWindow->Collapsed)
	{
		scale = 15.f;
		scale *= g.IO.Framerate / 100.f;

		bool new_minus = g.IO.MouseWheel < 0.0f;	
	...
	if (g.HoveredWindow && !g.HoveredWindow->Collapsed && (steps > 0 || g.IO.MouseWheelH != 0.0f))
	{
		...
		if (steps)
		{
			...
			else if (!g.IO.KeyCtrl && scroll_allowed)
			{
				...
				float MouseWheel = (minus ? -scroll : scroll) / scale;
				SetWindowScrollY(scroll_window, scroll_window->Scroll.y - MouseWheel * scroll_amount);
				--steps;
				if (steps < 0.f)
					steps = 0.f;
			}
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos, float radius)
{
	...
	if (hovered)
		window->DrawList->AddCircleFilled(center, ImMax(2.0f, radius), GetColorU32((held && hovered) ? ImGuiCol_CloseButtonActive : ImGuiCol_CloseButtonHovered), 9);
