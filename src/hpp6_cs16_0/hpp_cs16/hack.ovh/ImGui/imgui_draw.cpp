...
void ImDrawList::AddGradientRect(const ImVec2& a, const ImVec2& b, const ImColor& clr_a, const ImColor& clr_b)
{
	...
	for (float y = 0.f; y < size_y; y += 1.f)
	{
		for (float x = 0.f; x < size_x; x += 1.f)
		{
			const float f0 = y / size_y;
			const float f1 = g_Utils.fInterp(0.f, x, size_x, clr_a.Value.x - f0, clr_b.Value.x - (clr_b.Value.x * f0));
			const float f2 = g_Utils.fInterp(0.f, x, size_x, clr_a.Value.y - f0, clr_b.Value.y - (clr_b.Value.y * f0));
			const float f3 = g_Utils.fInterp(0.f, x, size_x, clr_a.Value.z - f0, clr_b.Value.z - (clr_b.Value.z * f0));
		
			const auto from = ImVec2(a.x + x, a.y + y);
			const auto to = ImVec2(from.x + 1.f, from.y + 1.f);
...
void ImDrawList::AddGradientBar(const ImVec2& a, const ImVec2& b, const ImColor& clr_a, const ImColor& clr_b)
{
	...
	for (float x = 0.f; x < size_x; x += 1.f)
	{
		for (float y = 0.f; y < size_y; y += 1.f)
		{
			const float f1 = g_Utils.fInterp(0.f, y, size_y, clr_a.Value.x , clr_b.Value.x);
			const float f2 = g_Utils.fInterp(0.f, y, size_y, clr_a.Value.y, clr_b.Value.y);
			const float f3 = g_Utils.fInterp(0.f, y, size_y, clr_a.Value.z, clr_b.Value.z);
			const float f4 = g_Utils.fInterp(0.f, y, size_y, clr_a.Value.w, clr_b.Value.w);
