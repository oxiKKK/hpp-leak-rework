...
double CNoSpread::GetConstantRemainder(Vector Spread) // aka Trap God Constant
{
	double ConstantRemainder = 0;

	Vector Forward, Right, Up;

	Vector SpreadAt90Pitch(Spread[1], -Spread[0], -1);
...
void CNoSpread::GetSpreadXY(unsigned int a_iRandomSeed, int a_iFuture, Vector &vecSpread, bool a_bSimulate)
{
	...
	if (a_bSimulate) {
		vecSpread[0] *= g_Weapon.m_flSpreadSimulated;
		vecSpread[1] *= g_Weapon.m_flSpreadSimulated;
	}
	else {
		vecSpread[0] *= g_Weapon.m_flSpread;
		vecSpread[1] *= g_Weapon.m_flSpread;
	}
}
...
void CNoSpread::GetSpreadOffset(unsigned int a_iRandomSeed, int a_iFuture, QAngle a_QInAngles, QAngle &a_QOutAngles, int a_nType)
{
	if (a_nType == 1) 
	{
		...
		GetSpreadXY(a_iRandomSeed, a_iFuture, vecSpread, false);
	...
	else if (a_nType == 3)
	{
		...
		GetSpreadXY(a_iRandomSeed, a_iFuture, vecRandom, false);
