...
ImGuiContext* ImGui::CreateContext(void* (__cdecl *malloc_fn)(size_t), void(__cdecl *free_fn)(void*))
{
...
{
	void(__cdecl *free_fn)(void*) = ctx->IO.MemFreeFn;
	ctx->~ImGuiContext();
	free_fn(ctx);
	if (GImGui == ctx)
...
void ImGui::NewFrame()
{
	...
	static bool bSmoothScrolling = false;
	static bool bInterpolated = false;
	static float flInterolateTime = 0;
	static float flScrollStart = 0.f;
	static float flScrollEnd = 0.f;
	static int HoveredWindowID = 0;
	const float flScrollStep = g.IO.MouseWheel;

	if (g.HoveredWindow)
	{
		if (HoveredWindowID != g.HoveredWindow->ID)
		{
			bSmoothScrolling = bInterpolated = false;
			flScrollStart = flScrollEnd = 0.f;
			HoveredWindowID = g.HoveredWindow->ID;
		}
	}

	if (!bSmoothScrolling && flScrollStep != 0.f && g.HoveredWindow && !g.HoveredWindow->Collapsed)
	{
		flInterolateTime = static_cast<float>(client_state->time);
		bSmoothScrolling = true;
	}

	if (g.HoveredWindow && !g.HoveredWindow->Collapsed && bSmoothScrolling)
	{
		...
		else if (!g.IO.KeyCtrl && scroll_allowed)
		{
			...
			if (!bInterpolated && flScrollStep != 0.0F)
			{
				flScrollStart = scroll_window->Scroll.y;
				flScrollEnd = flScrollStart - flScrollStep * scroll_amount;
				bInterpolated = true;
			}
			else if (bInterpolated && flScrollStep != 0.0F)
			{
				flScrollStart = scroll_window->Scroll.y;
				flScrollEnd -= flScrollStep * scroll_amount;
				flInterolateTime = static_cast<float>(client_state->time);
			}

			float flScrollLepred = g_Utils.fInterp(flInterolateTime, static_cast<float>(client_state->time),
				flInterolateTime + 0.3F, flScrollStart, flScrollEnd);

			if (flScrollLepred < 0.0F)
			{
				bSmoothScrolling = bInterpolated = false;
				flScrollStart = flScrollEnd = 0.0F;
				flScrollLepred = 0.0F;
			}
			else if (flScrollLepred > GetScrollMaxY(scroll_window))
			{
				bSmoothScrolling = bInterpolated = false;
				flScrollStart = flScrollEnd = 0.0F;
				flScrollLepred = GetScrollMaxY(scroll_window);
			}

			SetWindowScrollY(scroll_window, flScrollLepred);

			if (flInterolateTime + 0.3F < static_cast<float>(client_state->time))
			{
				bSmoothScrolling = bInterpolated = false;
				flScrollStart = flScrollEnd = 0.F;
			}
...
void ImGui::End()
{
	...
	CheckStacksSize(window, 1); // oxiKKK
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos, float radius)
{
	...
	if (hovered)
		window->DrawList->AddCircleFilled(center, ImMax(2.0f, radius), GetColorU32((held && hovered) ? ImGuiCol_CloseButtonActive : ImGuiCol_CloseButtonHovered), 25);
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	if (!strcmp(label, "##FileContent"))
		Menu::bUpdateFileContent = g.ActiveId != id;
...
void RenderCollapseTriangle(ImVec2 p_min, bool is_open, float scale)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	...
	if (is_open)
	{
		...
		b = center + ImVec2(-1, -1)*r;
		c = center + ImVec2(1, -1)*r;
	}
	else
	{
		center.y -= r * 0.25f;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImVec2 popup_size)
{
	...
	RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
	//RenderFrame (ImVec2 (frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, GetColorU32 (popup_open || hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button), true, style.FrameRounding); // FIXME-ROUNDING
	RenderCollapseTriangle(ImVec2(frame_bb.Max.x - arrow_size + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 4), !popup_open, 0.5f);
	...
	ImGuiWindowFlags window_flags = ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings;
	if (!Begin(name, NULL, window_flags))
	{
		EndPopup();
		IM_ASSERT(0);   // This should never happen as we tested for IsPopupOpen() above
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	{
		const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
		RenderFrame(bb_with_spacing.Min, bb_with_spacing.Max, col, false);
	}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	{
		// Render SV Square
		draw_list->AddGradientRect(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), ImColor(1.f, 1.f, 1.f), ImColor(hue_color_f));
		...
		// Render Hue Bar
		for (int i = 0; i < 6; ++i)
			draw_list->AddGradientBar(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)),
				ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)), ImColor(hue_colors[i]), ImColor(hue_colors[i + 1]));
	...
	if (alpha_bar)
	{
		...
		draw_list->AddGradientBar(bar1_bb.Min, bar1_bb.Max, ImColor(col32_no_alpha), ImColor(col32_no_alpha & ~IM_COL32_A_MASK));
...
void ImGui::Separator(float CustomWidth)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	float x2 = CustomWidth ? window->Pos.x + CustomWidth : window->Pos.x + window->Size.x;
