...
void ImDrawList::AddGradientRect(const ImVec2& a, const ImVec2& b, const ImColor& clr_a, const ImColor& clr_b)
{
	const float PIXEL_SIZE = 1.0F;

	const float size_x = fabs(a.x - b.x);
	const float size_y = fabs(a.y - b.y);

	for (float y = 0.0F; y < size_y; y += PIXEL_SIZE)
	{
		for (float x = 0.0F; x < size_x; x += PIXEL_SIZE)
		{
			const float f0 = y / size_y;
			const float f1 = g_Utils.fInterp(0.0F, x, size_x, clr_a.Value.x - f0, clr_b.Value.x - (clr_b.Value.x * f0));
			const float f2 = g_Utils.fInterp(0.0F, x, size_x, clr_a.Value.y - f0, clr_b.Value.y - (clr_b.Value.y * f0));
			const float f3 = g_Utils.fInterp(0.0F, x, size_x, clr_a.Value.z - f0, clr_b.Value.z - (clr_b.Value.z * f0));
		
			const auto from = ImVec2(a.x + x, a.y + y);
			const auto to = ImVec2(from.x + PIXEL_SIZE, from.y + PIXEL_SIZE);
			const auto color = ImColor(f1, f2, f3);

			AddRectFilled(from, to, color);
		}
	}
}

void ImDrawList::AddGradientBar(const ImVec2& a, const ImVec2& b, const ImColor& clr_a, const ImColor& clr_b)
{
	...
	for (float x = 0.0F; x < size_x; x += PIXEL_SIZE)
	{
		for (float y = 0.0F; y < size_y; y += PIXEL_SIZE)
		{
			const float f1 = g_Utils.fInterp(0.0F, y, size_y, clr_a.Value.x, clr_b.Value.x);
			const float f2 = g_Utils.fInterp(0.0F, y, size_y, clr_a.Value.y, clr_b.Value.y);
			const float f3 = g_Utils.fInterp(0.0F, y, size_y, clr_a.Value.z, clr_b.Value.z);
			const float f4 = g_Utils.fInterp(0.0F, y, size_y, clr_a.Value.w, clr_b.Value.w);

			const auto from = ImVec2(a.x + x, a.y + y);
			const auto to = ImVec2(from.x + PIXEL_SIZE, from.y + PIXEL_SIZE);
			const auto color = ImColor(f1, f2, f3, f4);
...
ImFont* ImFontAtlas::AddFontDefault(const ImFontConfig* font_cfg_template)
{
	...
	/* if (font_cfg.Name[0] == '\0') strcpy(font_cfg.Name, "Lucida.ttf, 10px");
    if (font_cfg.SizePixels <= 0.0f) font_cfg.SizePixels = 10.0f;*/

// font_change
   // ImFont* font = AddFontFromMemoryCompressedTTF (lucida_compressed_data, lucida_compressed_size, 10.f, &font_cfg, GetGlyphRangesCyrillic ());
	ImFontConfig fontConfig;
	fontConfig.GlyphOffset.y = -2;

	std::string str = getenv("appdata");
	std::size_t pos = str.find(":");
	std::string sFontPath = str.substr(0, pos);
	sFontPath.append(":\\Windows\\Fonts\\verdana.ttf");

	ImFont* font = AddFontFromFileTTF(sFontPath.c_str(), 13.f, &fontConfig, GetGlyphRangesCyrillic());

	//ImFont* font = AddFontFromFileTTF (std::string (g_Globals.m_szSettingsPath + "verdana.ttf").c_str (), 13.f, &fontConfig, GetGlyphRangesCyrillic ());

	/*if(!font)
		font = AddFontFromFileTTF("C:\\Windows\\Fonts\\verdana.ttf", 13.f, &fontConfig, GetGlyphRangesCyrillic());*/
